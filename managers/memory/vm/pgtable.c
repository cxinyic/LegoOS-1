        ...
        struct vm_area_struct *vma, struct task_struct *dst_task)
{
    ...
    virt = lego_pte_to_virt(pte);
    phys = lego_pte_to_phys(pte);
    // pr_debug("lego addr: %#Lx (COW? %d), pte va: %#Lx pa1: %#Lx pa2: %#Lx pfn: %#Lx, mm pa: %#Lx, page addr: %#Lx\n", addr, is_cow_mapping(vm_flags), virt, page_to_phys(virt_to_page(virt)), __pa(virt), virt_to_pfn(virt), phys, page_address(virt_to_page(virt)));

    pte = pfn_pte(virt_to_pfn(virt), vma->vm_page_prot);

    // // QZ: enforce the page to be writable, so the dcr t_context() can write the caller's memory
    pte = pte_mkyoung(pte);
    pte = pte_mkdirty(pte);
    pte = pte_mkwrite(pte);

pte_set:
    // pte_set(dst_pte, __pte(phys << PAGE_SHIFT | pgprot_val(vma->vm_page_prot)));
    // pte_set(dst_pte, pfn_pte(virt_to_pfn(virt), vma->vm_page_prot));
    pte_set(dst_pte, pte);
...
static int copy_pte_range(struct mm_struct *dst_mm,
    ...
    do {
        if (pte_none(*src_pte)) {
            // pr_debug("QZ: none pte at addr:%#Lx\n", addr);
            continue;
        }
        if (copy_one_pte(dst_mm, src_mm, dst_pte, src_pte, addr, vma, dst_task)) {
            ret = -ENOMEM;
            break;
        }
    } while (dst_pte++, src_pte++, addr += PAGE_SIZE, addr != end);
...
static inline int copy_pmd_range(struct mm_struct *dst_mm,
    ...
    do {
        next = pmd_addr_end(addr, end);
        if (pmd_none(*src_pmd)) {
            // pr_debug("QZ: none pmd at addr:%#Lx\n", addr);
            continue;
        }
...
static inline int copy_pud_range(struct mm_struct *dst_mm, struct lego_mm_struct *src_mm,
    ...
    do {
        next = pud_addr_end(addr, end);
        if (pud_none(*src_pud)) {
            // pr_debug("QZ: none pud at addr:%#Lx\n", addr);
            continue;
        }
    ...
    /*
     * This function copies the page table in lego_mm_struct to mm_struct
        ...
        struct vm_area_struct *vma, struct task_struct *tsk)
{
    ...
    do {
        next = pgd_addr_end(addr, end);
        if (pgd_none(*src_pgd)) {
            // pr_debug("QZ: none pgd at addr:%#Lx\n", addr);
            continue;
        }
